 ...
 *
 * This class is 100% not static. For standard debug logging, it is recommend **not** to use this
 * class, but to use class Log instead, which is a 100% static wrapper around this one. In other words,
...
public class Lox
{
    ...
    // #############################################################################################
        ...
        /**
         * The prefix of packages.classes.methods on the stack that are ignored when identifying the
         * caller info.
    ...
    // #############################################################################################
        ...
        /** A AString singleton. Can be acquired, using buf() */
    ...
    /** *********************************************************************************************
     * Returns a AString singleton, similar to the LogBuf singleton returned by Log.buf().
     * Whenever this method is called, the returned AString object has to be used as a message within
     ...
     * re-acquire the AString object by calling this method again prior to the next log call.
     *
    ...
    public  void addLogger( Logger logger, Log.DomainLevel internalDomainLevel )
    {
        //#if ALOX_DEBUG || ALOX_REL_LOG
            try { lock.acquire();
                ...
                AString internalDomain= new AString( this.internalDomain );
                LogDomain domain= logger.findDomain ( internalDomain );
                if ( domain == null )
                    domain= logger.createDomain( internalDomain );
                ...
                if ( loggers.contains( logger ) )
                {
                    internalLog( Log.Level.WARNING,    
                                 tempMS.clear().append("Lox.addLogger(): Logger \"")
                                               .append( logger.name )
                                               .append("\" already exists. Not added.") );
                    return;
                }
                ...
                internalLog( Log.Level.INFO,    
                             tempMS.clear().append("Lox.addLogger(): Logger \"")
                                           .append(logger.name).append("\" added.") );
        ...
        //#if ALOX_DEBUG || ALOX_REL_LOG
            try { lock.acquire();
                ...
                if ( scope != Log.Scope.NONE )
                {
                    ...
                    defaultDomains.put( key, new AString( domainToRegister ) );
     ...
     * @param recursive     Set sub domains recursively. Defaults to true.
     **********************************************************************************************/
    public void setDomain(String domain, Log.DomainLevel domainLevel, boolean recursive)
    {
        setDomain( domain, domainLevel, recursive, null );
    }
    ...
    public void setDomain(String domain, Log.DomainLevel domainLevel)
    {
        setDomain( domain, domainLevel, true, null );
    }
                ...
                for ( Logger logger : loggers )
                {
                    ...
                    internalLog( Log.Level.INFO,    
                                 tempMS.clear().append("Lox.setStartTime(): Start time of \"")
                                               .append(logger.name)
                                               .append("\" set to: ")
                                               .append(startTime!=null ? startTime.toString() : "\"now\"")
                                               .append('.')  );
                }
...

    {
        //#if ALOX_DEBUG || ALOX_REL_LOG
            try { lock.acquire();
                ...
                if ( !AString.isNullOrEmpty( origThreadName )  )
        ...
        //#if ALOX_DEBUG || ALOX_REL_LOG
            try { lock.acquire();
                ...
                internalLog( Log.Level.VERBOSE, 
                             tempMS.clear().append("Lox.setMarker(): Marker set (")
        ...
        //#if ALOX_DEBUG || ALOX_REL_LOG
            try { lock.acquire();
                ...
                else
                {
                    ...
                    if ( scope != Log.Scope.PACKAGE )        tempMS.append( '#' ).append( caller.className );
                    if ( scope == Log.Scope.METHOD )        tempMS.append( '#' ).append( caller.methodName );
                }
                ...
                // log info on this
                internalLog( Log.Level.VERBOSE, 
                             tempMS.clear().append("Lox.getMarker(): Marker retrieved (")
                                           .append(( marker != null ? marker.toString() : "null"))
                                           .append(").") );
     ...
     * @param indent    the indentation in the output.
     **********************************************************************************************/
    public void Assert(boolean trueOrLog, Object msg, int indent)
    {
        line( !trueOrLog, null, Log.Level.ERROR, msg, indent, null );
    }
     ...
     * @param msg       The message to log out.
     **********************************************************************************************/
    public void Assert(boolean trueOrLog, Object msg)
    {
        line( !trueOrLog, null, Log.Level.ERROR, msg, 0, null );
    }
     ...
     * @param indent    the indentation in the output.
     **********************************************************************************************/
    public void Assert(boolean trueOrLog, String domain, Object msg, int indent)
    {
        line( !trueOrLog, domain, Log.Level.ERROR, msg, indent, null );
    }
     ...
     * @param msg       The message to log out.
     **********************************************************************************************/
    public void Assert(boolean trueOrLog, String domain, Object msg)
    {
        line( !trueOrLog, domain, Log.Level.ERROR, msg, 0, null );
    }
        ...
        try
        {
            ...
            if (loggers.size() == 0)
            {
                ...
                internalLog(
                        Log.Level.WARNING,
                        tempMS.clear()
                              .append("Lox: Class 'Log' was used without prior creation of a Log instance. ConsoleLogger Logger created as default." ) );
            }
            ...
            // loop over all loggers
            for (Logger logger : loggers)
                if (simpleWildcardFilter( logger, loggerFilter )) logger.line( resDomain, level, msgObject, indent, caller );
    ...
    //#if ALOX_DEBUG || ALOX_REL_LOG
        ...
        protected  void evaluateResultDomain( String domain  )
        {
            ...
            boolean    noDomainGiven=    AString.isNullOrEmpty( domain );
            if ( noDomainGiven || domain.charAt( 0 ) == '~' )
            {
                ...
                if ( AString.isNullOrEmpty( defaultDomain ) )
                {
                    tempMS.setLength( strLengthClassScope );

                    defaultDomain= defaultDomains.get( tempMS );
                    if ( AString.isNullOrEmpty( defaultDomain ) )
                    {
                        tempMS.setLength( strLengthPkgScope );
                        defaultDomain= defaultDomains.get( tempMS );
                    }
                }
            }
                ...
                if ( AString.isNullOrEmpty( defaultDomain ) )
                {
                    internalLog( Log.Level.WARNING,
                                 tempMS.clear() .append("Lox: No log domain given and default domain not set for scope \"")
                                                .append(caller.packageName)
                                                .append('.').append(caller.className)
                                                .append('.').append(caller.methodName).append("\".")
                                );
                    return;
                }
                ...
                else
                {
                    ...
                    internalLog( Log.Level.WARNING,     
                                 tempMS.clear() .append("Lox: Relative domain path given: \"").append(domain)
                                                ...
                                                .append(caller.packageName).append('.')
                                                .append(caller.className).append('.')
                                                .append(caller.methodName).append("\".")  );
                }
            }
        ...
        private static  boolean simpleWildcardFilter( Logger logger, String loggerFilter )
        {
            ...
            if ( startWC )
            {
                // both?
                if ( endWC )
                    return AString.indexOfIgnoreCase( logger.name, loggerFilter ) >= 0;

                // at start only
                return  AString.endsWithIgnoreCase( logger.name, loggerFilter );
            }

            // at end
            if ( endWC)
                return AString.startsWithIgnoreCase( logger.name, loggerFilter );
