...
//  
//  Class:	LogTools
//  File:	LogTools.cs											  Namespace:  com.aworx.lox.tools
// #################################################################################################
...
public class LogTools 
{
		...
		/// <summary>Temporary helper buffer.</summary>
		protected	static		MString						tempBuf;
...

	{
		...
		{ 
			...
			{ 
				...
				if ( tempBuf == null )	  tempBuf= new MString( 64 );
...
//					else 
					{
						...
						toolBuf	.Append( FmtInstTypePrefix )
								.Append( "KeyValuePair<").Append( instKeyType	!= null ? instKeyType.Name	 : "null" )
								.Append(			", ").Append( instValueType != null ? instValueType.Name : "null" )
								.Append(			 ">")
								.Append( FmtInstTypePostfix );
			...
			/** ***********************************************************************************************
			 * <summary>
			 *  Detects if the given object is of value type. Special treatment is given to KeyValuePair&lt;,
			 *  &gt;. C# treat them as  a value type that causes some irritation sometimes.
			 * </summary>
			 * <param name="o">   	The object to be tested. </param>
			 * <param name="type">	The type of the object (needed outside, therefore passed here to avoid double
			 * 						creation). </param>
			 * <returns>	the value (if given object is of value type) or null otherwise. </returns>
			 **************************************************************************************************/
			protected static bool isValueType( Object o, Type type )
			{
				...
				if ( type == typeof(String) )
				{
					...
					tempBuf	.Append( '\"' ).Append( (String) o ).Append( '\"' );
					return true;
				}
				...
				if ( type == typeof( Char[]) )
				{
					...
					// add value and type, that's it
					tempBuf	.Append( '\"' ).Append( val ).Append( '\"' );
