...
public class LogDomain
{
     ...
     **********************************************************************************************/
    public LogDomain( LogDomain parent,  AString name )
    {
    ...
    /** ********************************************************************************************
     * <summary>
     *  Searches a domain. If the flag createIfNotExistent is set, then one or more sub domains are
     ...
     * <param name="createIfNotExistent">    If set true, domains in the path that are not found are
     *                                       created. </param>
     * <returns>    The domain found or created. </returns>
     **********************************************************************************************/
    public LogDomain FindOrCreate( AString domainPath, bool createIfNotExistent)
    {
        ...
        // call find
        return ld.findOrCreateInternal( domainPath, startIdx, createIfNotExistent );
    }
     ...
     * <param name="createIfNotExistent">    Create domain if not found on the path. </param>
     * <returns> The found domain. </returns>
     **********************************************************************************************/
    protected LogDomain findOrCreateInternal( AString    domainPath,    int dpStartIdx, bool createIfNotExistent)
    {
        ...
        if ( comparison != 0 )
        {
            ...
            // create domain
            subDomain= new LogDomain( this, new AString( domainPath, dpStartIdx, pEnd-dpStartIdx ) );
            SubDomains.Insert( idx, subDomain );
        }

        // recursion?
        if ( pNext > 0 )
            return subDomain.findOrCreateInternal( domainPath, pNext, createIfNotExistent );
