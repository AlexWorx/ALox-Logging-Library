...
class SimpleTests
{
    ...
    void performanceTest()
    {
        ...
        #if ALOX_DEBUG && AWORX_VSTUDIO
            cl.LineFormatter.ConsumableSourcePathPrefixString= String.Empty;
        #endif
        ...
        for ( int i= 0 ; i < 500 ; i++ )
        {
            ...
            if ( fastest > t )
            {
                ...
                Log.Line( Log.Level.Info, msgBuf.Clear().Append( "Pass " ).Append( i, 3).Append( " is new fastest:  ")
                                                        .Append( (int) (new Ticks( fastest)).InMillis(), 0).Append( " millis (").Append( (int) fastest ).Append( " ticks) per 1000 logs.") );
            }
        }

        Log.Line( Log.Level.Info, msgBuf.Clear().Append( "Fastest " ).Append( (int) (new Ticks( fastest)).InMillis(), 0).Append( " millis (").Append( (int) fastest ).Append( " ticks) per 1000 logs.") );
    }
    ...
    void classTextFileLogger()
    {
        Log.AddLogger( cl= new ConsoleLogger() );
        ...
        TextFileLogger tfl= new TextFileLogger( "Test_Text_Log.txt" );
        Log.AddLogger( tfl, Log.DomainLevel.WarningsAndErrors );

        Log.RegDomain( "TFLTest", Log.Scope.SourceFile );
        Log.SetDomain( "TFLTest", Log.DomainLevel.All );

        Log.Info( "Hello ALox TextFileLogger!" );
        Log.Info( "Noch ne Zeile" );
