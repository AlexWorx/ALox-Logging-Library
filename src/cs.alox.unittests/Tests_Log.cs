// #################################################################################################
//  com.aworx.lox.unittests - ALox Logging Library
//
...
namespace com.aworx.lox.unittests
{
    ...
    public void Log_TestTextLoggerTimeDiff()
    {
        ...
        t.TimeDiffMinimum= 0;
        diff= 0;                            ms.Clear(); t.t( ms, diff ); Assert.AreEqual(   "000" + t.TimeDiffNanos,   ms.ToString(0, ms.Length() ) );
        diff= 15;                           ms.Clear(); t.t( ms, diff ); Assert.AreEqual(   "015" + t.TimeDiffNanos,   ms.ToString(0, ms.Length() ) );
        diff= 99;                           ms.Clear(); t.t( ms, diff ); Assert.AreEqual(   "099" + t.TimeDiffNanos,   ms.ToString(0, ms.Length() ) );
        diff= 600;                          ms.Clear(); t.t( ms, diff ); Assert.AreEqual(   "600" + t.TimeDiffNanos,   ms.ToString(0, ms.Length() ) );
        diff= 999;                          ms.Clear(); t.t( ms, diff ); Assert.AreEqual(   "999" + t.TimeDiffNanos,   ms.ToString(0, ms.Length() ) );
        diff= 1000;                         ms.Clear(); t.t( ms, diff ); Assert.AreEqual(   "001" + t.TimeDiffMicros,  ms.ToString(0, ms.Length() ) );
        t.TimeDiffMinimum= 700;
        diff= 600;                          ms.Clear(); t.t( ms, diff ); Assert.AreEqual(           t.TimeDiffNone,    ms.ToString(0, ms.Length() ) );
        diff= 700;                          ms.Clear(); t.t( ms, diff ); Assert.AreEqual(   "700" + t.TimeDiffNanos,   ms.ToString(0, ms.Length() ) );
        diff= 999;                          ms.Clear(); t.t( ms, diff ); Assert.AreEqual(   "999" + t.TimeDiffNanos,   ms.ToString(0, ms.Length() ) );
        t.TimeDiffMinimum= 1000;
        diff= 1000;                         ms.Clear(); t.t( ms, diff ); Assert.AreEqual(   "001" + t.TimeDiffMicros,  ms.ToString(0, ms.Length() ) );
        diff= 15 * micros;                  ms.Clear(); t.t( ms, diff ); Assert.AreEqual(   "015" + t.TimeDiffMicros,  ms.ToString(0, ms.Length() ) );
        diff= 99 * micros;                  ms.Clear(); t.t( ms, diff ); Assert.AreEqual(   "099" + t.TimeDiffMicros,  ms.ToString(0, ms.Length() ) );
        diff= 600* micros;                  ms.Clear(); t.t( ms, diff ); Assert.AreEqual(   "600" + t.TimeDiffMicros,  ms.ToString(0, ms.Length() ) );
        diff= 999* micros;                  ms.Clear(); t.t( ms, diff ); Assert.AreEqual(   "999" + t.TimeDiffMicros,  ms.ToString(0, ms.Length() ) );
    ...
    public void Log_TestLogLevelSetting()
    {
        ...
        #else
            Assert.AreEqual ( (uint)  0, cl.CntLogs - logLinesBefore );
        #endif
    }
    ...
    public void Log_TestDefaultDomain()
    {
        ...
        cntLL= ml.CntLogs;    Log.Info   ( "/DFLT",        testOK  );    Assert.AreEqual ( oneLine,  ml.CntLogs - cntLL );
        ...
        cntLL= ml.CntLogs;    Log.Warning( "/DFLT",        testOK  );    Assert.AreEqual ( oneLine,  ml.CntLogs - cntLL );
        cntLL= ml.CntLogs;    Log.Warning( "/DFLT/WARN",   testOK  );    Assert.AreEqual ( oneLine,  ml.CntLogs - cntLL );
        cntLL= ml.CntLogs;    Log.Warning( "/DFLT/ERR",    testERR );    Assert.AreEqual ( (uint) 0, ml.CntLogs - cntLL );

        cntLL= ml.CntLogs;    Log.Error  ( "/DFLT",        testOK  );    Assert.AreEqual ( oneLine,  ml.CntLogs - cntLL );
        cntLL= ml.CntLogs;    Log.Error  ( "/DFLT/WARN",   testOK  );    Assert.AreEqual ( oneLine,  ml.CntLogs - cntLL );
        cntLL= ml.CntLogs;    Log.Error  ( "/DFLT/ERR",    testOK  );    Assert.AreEqual ( oneLine,  ml.CntLogs - cntLL );
        ...
        cntLL= ml.CntLogs;    Log.Info   (                 testOK  );    Assert.AreEqual ( oneLine,  ml.CntLogs - cntLL );
        cntLL= ml.CntLogs;    Log.Info   ( "~",            testOK  );    Assert.AreEqual ( oneLine,  ml.CntLogs - cntLL );
        ...
        cntLL= ml.CntLogs;    Log.Warning(                 testOK  );    Assert.AreEqual ( oneLine,  ml.CntLogs - cntLL );
        cntLL= ml.CntLogs;    Log.Warning( "~",            testOK  );    Assert.AreEqual ( oneLine,  ml.CntLogs - cntLL );
        cntLL= ml.CntLogs;    Log.Warning( "~WARN",        testOK  );    Assert.AreEqual ( oneLine,  ml.CntLogs - cntLL );
        cntLL= ml.CntLogs;    Log.Warning( "~ERR",         testERR );    Assert.AreEqual ( (uint) 0, ml.CntLogs - cntLL );

        cntLL= ml.CntLogs;    Log.Error  (                 testOK  );    Assert.AreEqual ( oneLine,  ml.CntLogs - cntLL );
        cntLL= ml.CntLogs;    Log.Error  ( "~",            testOK  );    Assert.AreEqual ( oneLine,  ml.CntLogs - cntLL );
        cntLL= ml.CntLogs;    Log.Error  ( "~WARN",        testOK  );    Assert.AreEqual ( oneLine,  ml.CntLogs - cntLL );
        cntLL= ml.CntLogs;    Log.Error  ( "~ERR",         testOK  );    Assert.AreEqual ( oneLine,  ml.CntLogs - cntLL );
    ...
    public void Log_TestAssertAndConditional()
    {
            ...
            cntLL= cl.CntLogs;    Log.Assert( false, testOK  );                          Assert.AreEqual ( oneLine,  cl.CntLogs - cntLL );
            cntLL= cl.CntLogs;    Log.Line  ( false, null, Log.Level.Info, testERR );    Assert.AreEqual ( (uint) 0, cl.CntLogs - cntLL );
            cntLL= cl.CntLogs;    Log.Line  ( true,  null, Log.Level.Info, testOK  );    Assert.AreEqual ( oneLine , cl.CntLogs - cntLL );
        #endif
    }
    ...
    public void Log_TestLineFormat()
    {
        ...
        AString  lf;
        lf= new AString( "%CF(%CL):%CM()%A3[%D][%TD][%TE +%TI] [%t] %L [%O] <%#>: ");   cl.LineFormatter.Format= lf;    Log.Info( "LineFormat set to= \"" + lf + "\"" );
        lf= new AString( "%CF(%CL):%A3[%D][%TD][%TE +%TI] [%t] %L [%O] <%#>: ");        cl.LineFormatter.Format= lf;    Log.Info( "LineFormat set to= \"" + lf + "\"" );
        lf= new AString( "%CF(%CL):%A3[%TD][%TE +%TI] [%t] %L [%O] <%#>: ");             cl.LineFormatter.Format= lf;    Log.Info( "LineFormat set to= \"" + lf + "\"" );
        lf= new AString( "%CF(%CL):%A3[%TE +%TI] [%t] %L [%O] <%#>: ");                  cl.LineFormatter.Format= lf;    Log.Info( "LineFormat set to= \"" + lf + "\"" );
        lf= new AString( "%CF(%CL):%A3[+%TI] [%t] %L [%O] <%#>: ");                      cl.LineFormatter.Format= lf;    Log.Info( "LineFormat set to= \"" + lf + "\"" );
        lf= new AString( "%CF(%CL):%A3[%t] %L [%O] <%#>: ");                             cl.LineFormatter.Format= lf;    Log.Info( "LineFormat set to= \"" + lf + "\"" );
        lf= new AString( "%CF(%CL):%A3%L [%O] <%#>: ");                                  cl.LineFormatter.Format= lf;    Log.Info( "LineFormat set to= \"" + lf + "\"" );
        lf= new AString( "%CF(%CL):%A3[%O] <%#>: ");                                     cl.LineFormatter.Format= lf;    Log.Info( "LineFormat set to= \"" + lf + "\"" );
        lf= new AString( "%CF(%CL):%A3[%O]: ");                                          cl.LineFormatter.Format= lf;    Log.Info( "LineFormat set to= \"" + lf + "\"" );
        lf= new AString( "%CF:%A3[%O]: ");                                               cl.LineFormatter.Format= lf;    Log.Info( "LineFormat set to= \"" + lf + "\"" );
        lf= new AString( "[%O]: ");                                                      cl.LineFormatter.Format= lf;    Log.Info( "LineFormat set to= \"" + lf + "\"" );
        lf= new AString( "");                                                            cl.LineFormatter.Format= lf;    Log.Info( "LineFormat set to= \"" + lf + "\"" );

        cl.LineFormatter.Format.Clear().Append("%D@");
        String df;
        df= "yyyy-MM-dd";   cl.LineFormatter.DateFormat= df;    Log.Info( "Date test. Format: \"" + df + "\"" );
        df= "yyy'Bla'MM'fasel'dd";     cl.LineFormatter.DateFormat= df;    Log.Info( "Date test. Format: \"" + df + "\"" );
