...
public class SimpleTests
{
        ...
        for ( int i= 0 ; i < 500 ; i++ )
        {
            ...
            if ( t.raw() < fastest )
            {
                ...
                Log.line( "CON", Log.Level.INFO,
                         Log.buf().append( "Pass " ).append( i, 3).append( " is new fastest:  ")
                                  .append( (int) t.inMillis(), 0).append( " millis (")
                                  .append( (int) fastest ).append( " ticks) per 1000 logs.") );
            }

        }

        Ticks f= new Ticks(fastest);
        Log.line( Log.Level.INFO,
                  Log.buf().append( "Fastest " ).append( (int) f.inMillis( ), 0)
                           .append( " millis (").append( (int) f.raw() )
                           .append( " ticks)  per 1000 logs.") );
    }
    ...
    public void testReleaseLogging()
    {
        ...
        LogTools.exception( "RelLog", Log.Level.ERROR,
                            new Exception(  "This is not a real Exception",
                                            new Exception( "...unreal inner") ),
                            " Logging instance 'this':", 0, lox );
