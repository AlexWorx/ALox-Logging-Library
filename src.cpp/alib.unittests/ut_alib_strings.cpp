...
namespace aworx { namespace lib { namespace strings {
    ...
    template<> struct T_Apply<myns::MyString> : public std::true_type
    {
        static inline integer Apply( AString& target, const myns::MyString& src )
        {
            if ( src.IsNulled() )
                return -1;
...
UT_METHOD( ConstructorsExplicit )
{
    ...
    const char*              testConstCharP=      "TEST";
          char*              testCharP=           const_cast<char*>( testConstCharP );
    ...
    const wchar_t*           testConstWCharP=     L"\u03B1\u03B2\u03B3\u03B4\u03B5"; // Greek characters;
          wchar_t*           testWCharP=          const_cast<wchar_t*>( testConstWCharP );
