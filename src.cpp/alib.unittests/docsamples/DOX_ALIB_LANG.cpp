...
{
    { EnumValue( Fruits::Apple  ), "Apple"  },
    { EnumValue( Fruits::Orange ), "Orange" },
    { EnumValue( Fruits::Banana ), "Banana" },
};
//! [DOX_ALIB_LANG_ENUMS_NORMAL_NAME_TABLE_DEFINITION]
...
EnumMetaData<MyNamespace::Pets>::GetSingleton()->Table=
{
    ...
    { EnumValue( Pets::Cat   ),   4    ,   0     ,  "Cat"   },
    { EnumValue( Pets::Dog   ),   4    ,   0     ,  "Dog"   },
    { EnumValue( Pets::Bird  ),   2    ,   2     ,  "Bird"  },
    { EnumValue( Pets::Snake ),   0    ,   0     ,  "Snake" },
};
//! [DOX_ALIB_LANG_ENUMS_CUSTOM_NAME_TABLE_DEFINITION]
...
EnumMetaData<WindowManager::States>::GetSingleton()->Table=
{
    ...
    {   EnumValue( WindowManager::States::HorizontallyMaximized
                 + WindowManager::States::VerticallyMaximized   )  ,"Maximized" },

    // The corresponding single bit entries.
    {   EnumValue( WindowManager::States::HorizontallyMaximized )  , "HMax"     },
    {   EnumValue( WindowManager::States::VerticallyMaximized   )  , "VMax"     },

    // Others...
    {   EnumValue( WindowManager::States::Hidden                )  , "Hidden"   },
};
//! [DOX_ALIB_LANG_ENUMS_BITWISE_DEFINITION]
...
Case target;
Substring("sensitive").ConsumeEnum( target ); cout << target << endl;
Substring("s"        ).ConsumeEnum( target ); cout << target << endl;
Substring("ignore"   ).ConsumeEnum( target ); cout << target << endl;
Substring("ign"      ).ConsumeEnum( target ); cout << target << endl;
//! [DOX_ALIB_LANG_ENUMS_PARSABLE_SAMPLE]
        UT_EQ( true , Substring("sensitive").ConsumeEnum( target ));   UT_TRUE( Case::Sensitive == target);
        UT_EQ( true , Substring("s"        ).ConsumeEnum( target ));   UT_TRUE( Case::Sensitive == target);
        UT_EQ( true , Substring("ignore"   ).ConsumeEnum( target ));   UT_TRUE( Case::Ignore    == target);
        UT_EQ( true , Substring("ign"      ).ConsumeEnum( target ));   UT_TRUE( Case::Ignore    == target);
        UT_EQ( false, Substring("hurz"     ).ConsumeEnum( target ));   UT_TRUE( Case::Ignore == target);
}
        ut.WriteResultFile( "DOX_ALIB_LANG_ENUMS_PARSABLE_OUTPUT.txt", testOutputStream.str(), "//! [OUTPUT]" );
        testOutputStream.str("");
...
//! [DOX_ALIB_LANG_ENUMS_BITWISE_SAMPLE]
        UT_EQ( "Normal"      , aworx::String64() << stateNull     )
        UT_EQ( "HMax"        , aworx::String64() << stateHM       )
        UT_EQ( "VMax"        , aworx::String64() << stateVM       )
        UT_EQ( "Maximized"   , aworx::String64() << stateHMVM     )
        UT_EQ( "HMax,Hidden" , aworx::String64() << stateHMHidden )
}
        ut.WriteResultFile( "DOX_ALIB_LANG_ENUMS_BITWISE_OUTPUT.txt", testOutputStream.str(), "//! [OUTPUT]" );
        testOutputStream.str("");
...
//! [DOX_ALIB_LANG_ENUMS_CUSTOM_SAMPLE]
    ...
    UT_EQ( "Cat"   , aworx::EnumReadWriteInfo<Pets>::Name( Pets::Cat   ))
    UT_EQ( "Dog"   , aworx::EnumReadWriteInfo<Pets>::Name( Pets::Dog   ))
    UT_EQ( "Bird"  , aworx::EnumReadWriteInfo<Pets>::Name( Pets::Bird  ))
    UT_EQ( "Snake" , aworx::EnumReadWriteInfo<Pets>::Name( Pets::Snake ))

}
        ut.WriteResultFile( "DOX_ALIB_LANG_ENUMS_CUSTOM_OUTPUT.txt", testOutputStream.str(), "//! [OUTPUT]" );
        testOutputStream.str("");
...
//! [DOX_ALIB_LANG_ENUMS_NORMAL_SAMPLE]
    UT_EQ( "Apple"  , aworx::String64() << fruit1);
    UT_EQ( "Orange" , aworx::String64() << fruit2);
    UT_EQ( "Banana" , aworx::String64() << fruit3);


}

        ut.WriteResultFile( "DOX_ALIB_LANG_ENUMS_NORMAL_OUTPUT.txt", testOutputStream.str(), "//! [OUTPUT]" );
        testOutputStream.str("");
...
(void) maximized;
    ...
    UT_METHOD( lang_enums_IterateBitwise )
    {
        ...
        UT_EQ( "One"   , buf << *bit     ); buf._();
        UT_EQ( "Two"   , buf << *(bit+1) ); buf._();
        UT_EQ( "Three" , buf << *(bit+2) ); buf._();
        UT_EQ( "Four"  , buf << *(bit+3) ); buf._();
        bit+=3;
        UT_EQ( "Four"  , buf << *bit     ); buf._();
        UT_EQ( "Three" , buf << *(bit-1) ); buf._();
        UT_EQ( "Two"   , buf << *(bit-2) ); buf._();
        UT_EQ( "One"   , buf << *(bit-3) ); buf._();
    ...
    UT_METHOD( lang_enums_Parseing )
    {
        ...
        caseRead= Case(-1);        s= "senitive"   ; UT_TRUE ( s.ConsumeEnum( caseRead ) );    UT_TRUE( caseRead== Case::Sensitive        ); UT_EQ( "itive"   , s);
        caseRead= Case(-1);        s= "ignore"     ; UT_TRUE ( s.ConsumeEnum( caseRead ) );    UT_TRUE( caseRead== Case::Ignore           ); UT_EQ( ""        , s);
        caseRead= Case(-1);        s= "abc"        ; UT_FALSE( s.ConsumeEnum( caseRead ) );    UT_TRUE( caseRead== Case(-1)               ); UT_EQ( "abc"     , s);

        ContainerOp copRead;
        copRead= ContainerOp(-1);  s= "G"          ; UT_TRUE ( s.ConsumeEnum( copRead  ) );    UT_TRUE( copRead == ContainerOp::Get       ); UT_EQ( ""        , s);
        copRead= ContainerOp(-1);  s= "Ge"         ; UT_TRUE ( s.ConsumeEnum( copRead  ) );    UT_TRUE( copRead == ContainerOp::Get       ); UT_EQ( ""        , s);
        copRead= ContainerOp(-1);  s= "GET"        ; UT_TRUE ( s.ConsumeEnum( copRead  ) );    UT_TRUE( copRead == ContainerOp::Get       ); UT_EQ( ""        , s);
        copRead= ContainerOp(-1);  s= "GETC"       ; UT_TRUE ( s.ConsumeEnum( copRead  ) );    UT_TRUE( copRead == ContainerOp::GetCreate ); UT_EQ( ""        , s);
        copRead= ContainerOp(-1);  s= "GETX"       ; UT_TRUE ( s.ConsumeEnum( copRead  ) );    UT_TRUE( copRead == ContainerOp::Get       ); UT_EQ( "X"       , s);

        BitsParseable bits;
        bits= BitsParseable(0); s= "onex"     ; UT_TRUE ( s.ConsumeEnumBitwise( bits) );      UT_TRUE( bits== BitsParseable::One   );      UT_EQ( "x"      , s);
        bits= BitsParseable(0); s= "murx"     ; UT_FALSE( s.ConsumeEnumBitwise( bits) );      UT_TRUE( bits== BitsParseable(0)     );      UT_EQ( "murx"   , s);
        bits= BitsParseable(0); s= "one,twox" ; UT_TRUE ( s.ConsumeEnumBitwise( bits) );      UT_TRUE( bits== BitsParseable::One
                                                                                                            + BitsParseable::Two   );      UT_EQ( "x"      , s);
        bits= BitsParseable(0); s= "f"        ; UT_TRUE ( s.ConsumeEnumBitwise( bits) );      UT_TRUE( bits== BitsParseable::Four  );      UT_EQ( ""       , s);
        bits= BitsParseable(0); s= "f , murx" ; UT_TRUE ( s.ConsumeEnumBitwise( bits)                                              );      UT_TRUE( bits== BitsParseable::Four   ); UT_EQ( ", murx" , s);
        bits= BitsParseable(0); s= "f , murx" ; UT_TRUE ( s.ConsumeEnumBitwise<BitsParseable ALIB_COMMA Case::Ignore ALIB_COMMA Whitespaces::Keep ALIB_COMMA ',' ALIB_COMMA true >( bits ));      UT_TRUE( bits== BitsParseable::Four   ); UT_EQ(" , murx" , s);
        bits= BitsParseable(0); s= "f , murx" ; UT_TRUE ( s.ConsumeEnumBitwise<BitsParseable ALIB_COMMA Case::Ignore ALIB_COMMA Whitespaces::Trim ALIB_COMMA ',' ALIB_COMMA false>( bits ));      UT_TRUE( bits== BitsParseable::Four   ); UT_EQ(   "murx" , s);
        bits= BitsParseable(0); s= "f, murx" ;  UT_TRUE ( s.ConsumeEnumBitwise<BitsParseable ALIB_COMMA Case::Ignore ALIB_COMMA Whitespaces::Keep ALIB_COMMA ',' ALIB_COMMA false>( bits ));      UT_TRUE( bits== BitsParseable::Four   ); UT_EQ(  " murx" , s);
    }
