...
UT_METHOD(Boxing_Dox)
{
    ...
    UT_PRINT("") UT_PRINT( "### Boxing Dox Samples ###" );
    {
        ...
        testOutputStream.str( ml.MemoryLog.NewLine().ToCString() );
        ut.WriteResultFile( "DOX_ALIB_BOXING_DEBUG.txt", testOutputStream.str(), "//! [OUTPUT]" );
    ...
    ut.WriteResultFile( "DOX_ALIB_BOXING_SAMPLE_1.txt", testOutputStream.str(), "//! [OUTPUT]" );
    testOutputStream.str("");

    dox_boxing_sample_2::sampleFunc();
    ut.WriteResultFile( "DOX_ALIB_BOXING_SAMPLE_2.txt", testOutputStream.str(), "//! [OUTPUT]" );
    testOutputStream.str("");

    dox_boxing_sample_2::sampleFunc2();
    ut.WriteResultFile( "DOX_ALIB_BOXING_SAMPLE_2_3.txt", testOutputStream.str(), "//! [OUTPUT]" );
    testOutputStream.str("");


    dox_boxing_bijective::sampleFunc();
    ut.WriteResultFile( "DOX_ALIB_BOXING_SAMPLE_BIJ.txt", testOutputStream.str(), "//! [OUTPUT]" );
    testOutputStream.str("");


    dox_boxing_sample_arr::sampleFunc();
    ut.WriteResultFile( "DOX_ALIB_BOXING_SAMPLE_ARR.txt", testOutputStream.str(), "//! [OUTPUT]" );
    testOutputStream.str("");

    dox_boxing_sample_arr::sampleFunc2();
    ut.WriteResultFile( "DOX_ALIB_BOXING_SAMPLE_ARR_2.txt", testOutputStream.str(), "//! [OUTPUT]" );
    testOutputStream.str("");


    dox_boxing_sample_ftypes::sampleFunc();
    ut.WriteResultFile( "DOX_ALIB_BOXING_SAMPLE_FTYPES_1.txt", testOutputStream.str(), "//! [OUTPUT]" );
    testOutputStream.str("");

    dox_boxing_sample_boxedas::sampleFunc();
    ut.WriteResultFile( "DOX_ALIB_BOXING_SAMPLE_BOXEDAS.txt", testOutputStream.str(), "//! [OUTPUT]" );
    testOutputStream.str("");

    dox_boxing_sample_if::sampleFunc();
    ut.WriteResultFile( "DOX_ALIB_BOXING_SAMPLE_IF_1.txt", testOutputStream.str(), "//! [OUTPUT]" );
    testOutputStream.str("");

    dox_boxing_sample_strings::sampleFunc();
    ut.WriteResultFile( "DOX_ALIB_BOXING_SAMPLE_STRINGS.txt", testOutputStream.str(), "//! [OUTPUT]" );
    testOutputStream.str("");

    dox_boxing_sample_recipe::sampleFunc();
    ut.WriteResultFile( "DOX_ALIB_BOXING_SAMPLE_RECIPE_1.txt", testOutputStream.str(), "//! [OUTPUT]" );
    testOutputStream.str("");
    ...
    ut.WriteResultFile( "DOX_ALIB_BOXING_BOXES_SAMPLE_VARIADIC_CALL_1.txt", testOutputStream.str(), "//! [OUTPUT]" );
    testOutputStream.str("");
    dox_boxing_sample_boxes::sampleFunc3();
    ut.WriteResultFile( "DOX_ALIB_BOXING_BOXES_SAMPLE_VARIADIC_CALL_2.txt", testOutputStream.str(), "//! [OUTPUT]" );
    testOutputStream.str("");

    dox_boxing_sample_boxes_box::sampleFunc();
    ut.WriteResultFile( "DOX_ALIB_BOXING_BOXES_SAMPLE_VARIADIC_SINGLE.txt", testOutputStream.str(), "//! [OUTPUT]" );
    testOutputStream.str("");


    dox_boxing_sample_debug::sampleFunc();
    ut.WriteResultFile( "DOX_ALIB_BOXING_DEBUG_1.txt", testOutputStream.str(), "//! [OUTPUT]" );
    testOutputStream.str("");

    dox_boxing_sample_debug::sampleFunc2();
    ut.WriteResultFile( "DOX_ALIB_BOXING_DEBUG_2.txt", testOutputStream.str(), "//! [OUTPUT]" );
    testOutputStream.str("");

    dox_boxing_sample_debug::sampleFunc3();
    ut.WriteResultFile( "DOX_ALIB_BOXING_DEBUG_3.txt", testOutputStream.str(), "//! [OUTPUT]" );
    testOutputStream.str("");
...
UT_METHOD(Boxing_Strings)
{
    ...
    // Equals: char*/char[]
    {
        ...
        Box falseP( static_cast<const char*>("not hello"));
        Box falseA(                          "not hello");
        AString asHello( "Hello" );
        AString asFalse( "not hello" );
        Box helloAS( asHello );
        Box falseAS( asFalse );
...
UT_METHOD(Boxing_Std_String)
{
    ...
    UT_PRINT("") UT_PRINT( "### Boxing: std::string ###" );
    {
        ...
        Box b( stdstr );
        UT_EQ( true, b.IsArrayOf<char>() );
        UT_EQ( 5,    b.Length()         );
        UT_EQ( 'H',  b.UnboxElement<char>(0) );

        std::string* stdstrP = &stdstr;
        b= Box( stdstrP );
        UT_EQ( true, b.IsArrayOf<char>() );
        UT_EQ( 5,    b.Length()         );
        UT_EQ( 'H',  b.UnboxElement<char>(0) );
...
UT_METHOD(Boxing_Std_Vector)
{
    ...
    }
    {
        ...
        Box b(vec);
        UT_EQ( true, b.IsArrayOf<std::string>() );
        UT_EQ( 3,    b.Length()         );
        UT_EQ( std::string("one"),    b.UnboxElement<std::string>(0) );
        UT_EQ( std::string("two"),    b.UnboxElement<std::string>(1) );
        UT_EQ( std::string("three"),  b.UnboxElement<std::string>(2) );
        std::string* array= &b.UnboxElement<std::string>(0);
        UT_EQ( std::string("one"),    array[0] );
        UT_EQ( std::string("two"),    array[1] );
        UT_EQ( std::string("three"),  array[2] );
    }
    ...
    {
        bool result;
        std::vector<char> helloVec= {'H','e','l','l','o'};
        std::vector<char> falseVec= {'n','o','t',' ','H','e','l','l','o'};
        Box helloV( helloVec );
        Box helloA( "Hello");
        Box falseP( falseVec );
        Box falseA( "not hello");
...
UT_METHOD(Boxing_Boxes)
{
...

    {
        ...
        boxes.clear(); boxes.Add( 5, "xyz" );              UT_EQ( static_cast<size_t>(2),  boxes.size() );  UT_EQ(   "xyz", boxes[1].Unbox<String   >());
        boxes.clear(); boxes.Add( 5, String64("xyz") );    UT_EQ( static_cast<size_t>(2),  boxes.size() );  UT_EQ(   "xyz", boxes[1].Unbox<String   >());
        boxes.clear(); boxes.Add("Hello ", "World" );      UT_EQ( static_cast<size_t>(2),  boxes.size() );  UT_EQ( "World", boxes[1].Unbox<String   >());

        Boxes boxes2("List1", "List2" );
        boxes.Add( boxes2 );                               UT_EQ( static_cast<size_t>(4),  boxes.size() );
        boxes.Add( Box(boxes2) );                          UT_EQ( static_cast<size_t>(6),  boxes.size() );
        UT_EQ( "Hello ", boxes[0].Unbox<String   >());
        UT_EQ( "World" , boxes[1].Unbox<String   >());
        UT_EQ( "List1" , boxes[2].Unbox<String   >());
        UT_EQ( "List2" , boxes[3].Unbox<String   >());
        UT_EQ( "List1" , boxes[4].Unbox<String   >());
        UT_EQ( "List2" , boxes[5].Unbox<String   >());
    }
}
...
{
    asTestIApply.Clear();
    UT_TRUE( box.HasInterface<IApply>() );
    box.Invoke<IApply>( asTestIApply );
...
UT_METHOD(Boxing_Interface_IApply)
{
    ...
    testIApply(ut, 5.2, "5.2" );
    testIApply(ut, 5U,  "5" );
    testIApply(ut,  5,  "5" );
    testIApply(ut, "Hello", "Hello" );
    testIApply(ut, TString("atstring"),  "atstring");
    testIApply(ut, AString("anastring"),  "anastring");
    testIApply(ut, String64("string64"),  "string64");
}
