...
void PerformanceTest()
{
    ...
    for ( int i= 0 ; i < qtyLoops ; i++ )
    {
        ...
        {
            fastest= t.Raw();
            Log_Info( "/CON", "Pass {:03} is new fastest: {:4} micros per {} logs.", i, t.InMicros(),  qtyLines );
        }
    }

    Log_Prune( double microsPerLog=  static_cast<double>((Ticks(fastest)).InMicros() ) / qtyLines; )
