 ...
 * that is \"\alib{strings,T_Apply,applicable}\" to objects of class \alib{strings,AString}.
 ...
 * class \alib{strings::format,PropertyFormatterMapApplicable} to objects of class \alib{strings,AString}.
...
class PropertyFormatters
{
    protected:
        ...
        std::map< TVariable, PropertyFormatter<TFormattable>* >         formatters;

        /** The callback table for the property formatters. (As given in the constructor.) */
        typename PropertyFormatter<TFormattable>::TCallbackTable &      callbacks;

        /** The base formatter to use. (As given in the constructor.) */
    ...
    public:
         ...
         *                       Defaults to \c nullptr which denotes to use method
         *                       \alib{strings::format,Formatter::AcquireDefault} to get the
         *                       default formatter.
         */
        PropertyFormatters( typename PropertyFormatter<TFormattable>::TCallbackTable&  callbackTable,
                ...
                for( auto& varEntry : varMetaData->Table )
                {
                    ...
                    if( varEnum == option )
                    {
                        if(    lib::lang::T_Resourced<TVariable>::Lib().Config->Load( var.Declare( varEnum ) ) == Priorities::NONE
                            ...
                            throw Exception(ALIB_SRCPOS_REL_NULLED, Exceptions::MissingConfigurationVariable,
                                            var.Fullname );

                        formatter= formatters.insert( std::make_pair(option, new PropertyFormatter<TFormattable>( var.GetString(), callbacks, baseFormatter) ) ).first;
                        break;
                    }
                }
 ...
 */
template<typename TFormattable, typename TOptionEnum >
struct       T_Apply<typename format::PropertyFormatterMapApplicable<TFormattable,TOptionEnum> > : public std::true_type
{
    /**
     ...
     * @return Constant \p 1.
     */
    static integer Apply( AString& target, typename format::PropertyFormatterMapApplicable<TFormattable,TOptionEnum>& src )
    {
