...
class ResourcedTupleLoader
{
    ...
    // #############################################################################################
         ...
         */
        static void readElement( char&            e, Substring& src )
        {
            e= src.ConsumeChar();
        }
        ...
        {
            src.ConsumeInt( e );
            ALIB_ASSERT_ERROR( src.IsEmpty(),    "Error in tuple resource data." )
        }
    ...
    public:
         ...
         *   Read 'as is' from the resource string.
         * - Integer types:<br>
         *   Read using singleton \alib{strings,NumberFormat::Computational} which
                               ...
                               char                    delim= ','                              )
        {
            ...
            else
            {
                ...
                ALIB_ASSERT_ERROR( tuples.size() > 0,
                                   ...
                                   "\" found in category: ", library.ResourceCategory );
            #else
                #if ALIB_DEBUG
                    if( tuples.size() == 0 )
                    {
                        ...
                        ALIB_ERROR( "No tuple resource \""  , tName.ToCString(),
                                    "\" found in category: ", tCategory.ToCString() );
                    }
                #endif
            #endif
        }
