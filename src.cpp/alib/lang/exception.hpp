 ...
 * - \alib{strings::format,Formatter,ALib Formatters}.
 *
 \~Comment ####################################################################################### \~
 ...
 * \ref ALIB_SRCPOS_REL_EMPTY. If the source information should still be
 * available in release compilations, then alternatively macro \ref ALIB_SRCPOS
 ...
 *   \alib{strings::format,SimpleText} or using
 *   \ref aworx::lox::LogTools::Exception "ALox LogTools" to nicely format the output).
 *
 * Here, a conflict may arise: Method #Describe passes all optional parameters
 * to a \alib{strings::format,Formatter} along with a format string loaded from
 ...
 *     \alib{strings::format,SimpleText::AddException} may be used.
...
class Exception
{
    ...
    public:
        ...
        struct Entry
        {
             ...
             */
            Entry( const TString& file, int line, const TString& func,
    ...
    public:
        ...
        template <typename TEnum,
                  typename... TArgs  >
        Exception( const TString& file, int line, const TString& func,
    ...
    // #############################################################################################
        ...
        template <typename TEnum,
                  typename... TArgs  >
        inline
        Exception&   Add( const TString& file, int line, const TString& func,
                          TEnum errEnum, TArgs&&... args )
        {
...
typename std::enable_if< std::is_same< typename T_EnumMetaDataDecl<TEnum>::TTuple,
                         ...
                         Exception&
                       >::type
AddFromBoxes( const TString& file, int line, const TString& func, TEnum errEnum, Boxes&& args )
//! [DOX_ALIB_ENUM_META_DATA_SPECIFCATION_usinginlib]
        {
