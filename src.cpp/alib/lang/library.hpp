 ...
 * - An application <b>"MyApp"</b> uses \alox logging library, which depends on \alib, as documented with \alox
 *   library class \ref #aworx::lox::ALox
 ...
 *   specific variables  separated from the main configuration files' documentation.
 *
 * What has to be done is:
 * - Initialize \alox with level
 *   \alib{lang,Library::InitLevels::PrepareConfig,InitLevels::PrepareConfig}
 * - Add a custom configuration plug-in (e.g \alib{config,IniFile}) to singleton
 *   #aworx::lox::ALOX.
 * - Initialize \alox with level \alib{lang,Library::InitLevels::All,InitLevels::All}
 *
 * Now \alox and all underlying \alib modules are initialized and all share the same configuration
 ...
 * - Make library singleton \ref #aworx::lox::ALOX a dependency library of <b>"MyApp"</b>.
 ...
 *   #aworx::lox::ALOX.
 * - Initialize \alox with level \alib{lang,Library::InitLevels::All,InitLevels::All}.<br>
 *   (Now \alox is fully initialized with \alox and \alib sharing their configuration.)
 *
 * - Initialize <b>"MyApp"</b> with level
 *   \alib{lang,Library::InitLevels::PrepareConfig,InitLevels::PrepareConfig}
...
class Library
{
    ...
    public:
         ...
         */
         strings::TString               ResourceCategory;
        ...
        /**
         * This is for creation output on information about the bits found in
    ...
    public:
        ...
        #else
        inline
        void                ReplaceResource( const TString& name, const String& data
                                   ALIB_DBG(,const String& expectedResource= NullString ) )
        {
            ALIB_ASSERT_ERROR( expectedResource.IsNull() || expectedResource.Equals( Get(name) ),
                               "Failure when replacing resource: Expected original value not found. "
                               "In resource: ",
                               ResourceCategory.ToCString(), " / ", name.ToCString() );
         ...
         *   the beginning of method method #init with \alib{lang::Library,Phases::resourceset}.
         *
         * In case they are different in a way that \b %ALib gets incompatible (e.g. different
...
template<> struct aworx::lib::lang::T_Resourced<TResourced>  :  public std::true_type              \
{                                                                                                  \
