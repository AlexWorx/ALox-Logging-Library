...
\note
    ...
    code using \b %ALib, this offers an easy way to disallow users of your software (which
...
{
    TrueValues= { "1", "true", "t", "yes", "y", "on", "ok" };

    if( addDefaultPlugins )
    {
        InsertPlugin( new InMemoryPlugin(CONFIG.Get("CfgPlgDef")), Priorities::DefaultValues   , Responsibility::Transfer);
        ...
        InsertPlugin( new InMemoryPlugin(CONFIG.Get("CfgPlgPro")), Priorities::ProtectedValues , Responsibility::Transfer);
    }
}
