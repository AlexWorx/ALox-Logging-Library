...
/** ************************************************************************************************
This is namespace documentation includes a lengthy introduction and tutorial section.

 Contents of the <b>reference documentation</b> section:
...
Classes found in this namespace comprise what is called a \b 'module' of the \b ALib class library.
The term "module" here means, that this feature of \b %ALib may be used independently from
...
Besides using this module or the full \b %ALib class library, there is a third option, which is
found with
[ALib %Boxing & Strings](https://github.com/AlexWorx/ALib-Boxing-And-Strings). This is is
module <b>ALib %Boxing</b> packaged together with module
...
might not want to use more of \b %ALib.
...
The original motivation to implement <b>ALib %Boxing</b> was the need to forward an arbitrary amount
...
need to discuss another important detail of the implementation of <b>ALib %Boxing</b>. This detail
...
With <b>ALib %Boxing</b> this type is implemented with class \ref aworx::lib::boxing::Boxer "Boxer".
...
Heap allocations in such situations are strict "no-go" for the \b %ALib library.
...
Besides this box value <b>ALib %Boxing</b> stores a second value that is called \b %Length.
...
Mathematically spoken, <b>ALib %Boxing</b> implements a <b>non-bijective</b> mapping of
...
Reconsidering the goals of <b>ALib %Boxing</b>, this makes a lot of sense and still
<b>ALib %Boxing</b> is type safe!
...
custom type \b MyClass was boxed without giving explicit instructions to <b>ALib %Boxing</b> on
...
- In the area of boxing arbitrary string types (\c char*, \c std::string, \c QTString,
  ...
  write methods that process extern types which are not "known" to them!
...
As already mentioned, <b>ALib %Boxing</b> does not provide a similar solution for multi-dimensional
...
The special treatment of arrays with <b>ALib %Boxing</b> imposes advantages and disadvantages and
...
- One-dimensional arrays can be accessed in a type safe way: <b>ALib %Boxing</b> distinguishes
...
<b>ALib %Boxing</b> provides built-in support for class \c std::vector. Objects of the type
...
The following code snippet is taken from header file \c "boxing.hpp" and is all that is needed.
...
\snippet "alib/boxing/boxing.hpp"    DOX_ALIB_BOXING_STD_VECTOR
...
Thus, module <b>ALib %Boxing</b> allows to set a custom version in place that
...
\note
    This freedom of choice is \b limited depending on the distribution of <b>ALib %Boxing</b>.
...
Method \b %useBox of the sample checks and processes only <b>12 boxed types</b>, while it is
able to 'fetch' <b>34 C++ fundamental types</b>. This demonstrates that the fundamental
...
- Types \c char, \c wchar_t, \c char16_t and \c char32_t remain identical.
- Pointers to these four built-in character types, are boxed to arrays of those types.
...
Designing <b>ALib %Boxing</b>, the question came up if it should still be allowed to receive
...
copied into it. This is not in alignment with the goals of <b>ALib %Boxing</b> and therefore not
implemented.
...
The only restriction is that no \p nullptr values must be passed.
...
of <b>ALib Boxing</b> is \e undermined this way: Methods that accept boxes as parameters and
...
In the previous chapters it was shown how <b>ALib %Boxing</b> uses C++ techniques like
...
To reach this goal, <b>ALib %Boxing</b> enables the definition and invocation of virtual methods
...
Module <b>ALib %Boxing</b> introduces class
...
the fact that <b>ALib %Boxing</b> is not bijective as explained
...
with module <b>ALib Strings</b> if packaged together with <b>ALib %Boxing</b>. This specialization
of interface class
...
in descendant classes. <b>ALib %Boxing</b> supports this concept with method
...
\ref aworx::lib::boxing::DefineDefaultInterface "DefineDefaultInterface" which is a convenience
function and internally invokes the first method named.

A useful sample is found with module <b>ALib Strings</b>. If packaged together with
<b>ALib %Boxing</b>, interface class
\ref aworx::lib::strings::boxing::IApply "IApply" becomes available. Specializations of
this interface are supposed to append the contents of a box to a given
\ref aworx::lib::strings::AString "AString". If no type specific version is registered, the
...
documented with each custom interface and is considered "static" piece of information that does
never change. If a default implementation is provided, the corresponding interface can be invoked
without prior checking for the availability. If an instance of class \b %Box named \b myBox exists,
...
\ref aworx::lib::boxing::Box::HasInterface  "Box::HasInterface" must not even be used for checking,
...
\note
  A quick warning: <b>ALib %Boxing</b> is no programming language: It neither provides inheritance, nor
  ...
  it is language and words that we understand and therefore we use it, knowing that we are talking
  on boxes not on objects!

<b>ALib %Boxing</b> interface classes which are available as "default interfaces" may be compared to
...
of a box equals the given parameter. The parameter to provided is declared as <c>const %Box%</c>.
...
- \ref aworx::lib::boxing::ftypes::IEquals_TcharArr    "IEquals_TcharArr" and
  \ref aworx::lib::boxing::ftypes::IEquals_Twchar_tArr "IEquals_Twchar_tArr" to character arrays.
...
The description of <b>ALib %Boxing</b> so far showed that - while the core concept is straightforward,
...
allow to change such in later header files. Therefore, \b %ALib
...
This way, a user of <b>ALib Boxing</b> might replace the default interfaces to suit an applications'
...
For example, literal <c>"ABC"</c> has type \c char[4] or <c>u"12345"</c> has type <c>char16_t[6]</c>).
...
We conclude all assumtions and rules as follows:
...
These rules apply to the following parts of <b>ALib %Boxing</b>
...
This is why the use of complete \b %ALib or combined module <b>ALib %Boxing and Strings</b> are
...
\note
  It is important to understand, that \b %ALib assumes that boxed pointers to character arrays are
...
for unboxing character arrays and pointers to characer types.
...
In the moment <b>ALib %Boxing</b> is used in combination with module
\ref aworx::lib::strings "ALib Strings", classes
\ref aworx::lib::strings::String "aworx::String" and
\ref aworx::lib::strings::String "aworx::AString" become available. The first is a "lightweight" class:
...
of class
\ref aworx::lib::strings::AString "AString", of-course the object itself is lost and is not
...
Class \ref aworx::lib::strings::String "AString" already supports a TMP-based mechanism to append
any sort of custom objects in a string representation (see
\ref aworx::lib::strings::T_Apply "T_Apply"). However, for boxing we can not use this.
...
For all types which already implement \ref aworx::lib::strings::T_Apply "T_Apply",
...
2. Make the type \e "applicable" to class \b %AString (as instructed
   ...
   generic interface \b %IApply_TApplicable to the <b>ALib %Boxing</b> system using the following
...
%Box interface
\ref aworx::lib::strings::boxing::IApply "IApply", introduced in the previous chapter, is used
...
"lightweight" class
\ref aworx::lib::strings::String  "aworx::String" and rather "heavy" class
\ref aworx::lib::strings::AString "aworx::AString" are boxed.
...
- Classes \c std::string, \c std::wstring, \c std::u16string and \c std::u32string.<br>
  (These types may not really be named "3rd party", but from the \b %ALib perspective, they
...
The version of the macro with suffix \p "_CB_CUB" specializes struct \b %T_Boxing in a way that
...
\ref aworx::lib::strings::boxing::IApply "IApply" for values of this \b BoxedAs type.
A ready to use templated incarnation of interface \b IApply exists with class
...
\ref aworx::lib::strings::T_Apply "T_Apply". But this support is suggested to be added anyhow for
...
functions with <b>ALib %Boxing</b>:
...
gets with <b>ALib %Boxing</b>. The recipe given, uses a single line of code to let the compiler
...
A real world sample can be found in the logging library \ref aworx::lox "ALox" which is
built on \b %ALib and makes a lot of use of <b>ALib %Boxing</b>.
While straightforward methods
\ref aworx::lox::Lox::Info    "Lox::Info",
\ref aworx::lox::Lox::Verbose "Lox::Verbose", etc. accept variadic template arguments as objects
to be logged, method
...
To make <b>ALib %Boxing</b> available in a compilation unit, header file \c "alib/alib.hpp" has
to be included. Depending on the \b %ALib distribution that is used (complete \b %ALib vs. the use
of just a module of \b %ALib that incorporates the boxing facilities), this header will
...
For bootstrapping <b>ALib Boxing</b> all that is needed to do is to invoke method
\ref aworx::lib::ALIB::Init "ALIB::Init" on bootstrap. This method will do the bootstrapping
for all alib portions that are included in the \b %ALib distribution.

\note
  As explained \ref aworx::lib "here", currently the full \b %ALib distribution is bundled together
  with the <b>%ALox Logging Library</b>, hence as a replacement, method
  \ref aworx::lox::ALox::Init "ALox::Init" might be invoked (which in turn invokes \b %ALIB::Init).

The following shows the excerpt from method \b %ALIB::Init which considers boxing:
\snippet "alib/alib.cpp"     DOX_ALIB_BOXING_BOOTSTRAP


We see three invocations of namespace functions:
1. \ref aworx::lib::boxing::Init<br>
   It is obvious, that this is the only function to be invoked if pure module
   [ALib %Boxing](https://github.com/AlexWorx/ALib-Boxing) is used.

2. \ref aworx::lib::strings::boxing::Init<br>
   This function needs to be invoked with combined module
   [ALib %Boxing & Strings](https://github.com/AlexWorx/ALib-Boxing-And-Strings).

3. \ref aworx::lib::time::InitBoxing<br>
   This function enables boxing of types found in namespace #aworx::lib::time.
...
types and string types of the [QT Class Library](https://www.qt.io).
...
using <b>ALib %Boxing</b>. We do this in a rather loose order:
...
### 8.4.7 Compile Times
There is another negative 'performance' impact when using <b>ALib %Boxing</b>: Due to the
use of TMP methods and need for the inclusion of standard header \c \<type_traits\> that provides
support for that, the time to compile a code unit increases with the use of <b>ALib %Boxing</b>.
...
### 8.4.8 Conclusion
Honestly, we consider the implementation of <b>ALib %Boxing</b> to be as performant as possible.
...
the setup of <b>ALib %Boxing</b>.  Class \b %Box provides method

- \ref aworx::lib::boxing::Box::DbgGetBoxerType "Box::DbgGetBoxerType"

which returns the type of the boxer that the box points to. From this type, the content type of
the box can be deduced. Also, the type can be passed to static method
...
Then, class \b %Boxer exposes the static methods used to get information about the overall
setup of <b>ALib %Boxing</b>:
...
When module <b>ALib %Boxing</b> is used with a distribution that includes module <b>ALib Strings</b>,
...
- <b>ALib %Boxing</b> is a free C++ software library that provides runtime type information and
...
- The central class of <b>ALib %Boxing</b> is class
...
- Depending on the configuration and distribution module of \b %ALib, built-in support for
...
- User defined types (e.g. structs and classes) are supported without the need of adding type
  specific  code to <b>ALib %Boxing</b>. The boxed type by default is a pointer type of the
  original type.
...
- Reasons for extending <b>ALib %Boxing</b> to add support for custom types are:
...
- <b>ALib %Boxing</b> is very valuable when functions or methods that should accept arbitrary
...
<b>Other Notes:</b><br>
- <b>ALib %Boxing</b> needs to be \ref alib_namespace_boxing_usage_bootstrap "bootstrapped"
...
- \ref  alib_namespace_boxing_usage_dump "Debug facilities" are provided that allow retrospection
  of the setup of <b>ALib %Boxing</b> in respect to types that were boxed and interfaces that
...
//! @cond NO_DOX
lang::TypeinfoMap<Boxer*>       Boxer::dbgKnownBoxers;
lang::TypeinfoMap<Interface*>   Boxer::dbgKnownInterfaces;
lang::TypeinfoMap<Interface*>   Boxer::dbgKnownInterfaceImpl;
//! @endcond NO_DOX
...
bool IEquals::Invoke( const Box& box, const Box& comp )
{
    ...
    if( !box.IsArray() )
        return      box.IsSameType( comp )
                &&  box.UnboxRaw() == comp.UnboxRaw()
                &&  box.Length() == comp.Length();

    // array types
    if(   !box.IsSameType( comp )
        || box.Length() != comp.Length()
        || ( (box.UnboxRaw() == 0)  != ( comp.UnboxRaw() == 0 ) )   )
        return false;
